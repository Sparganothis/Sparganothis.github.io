...
if WANDB_LOGGING:
    ...
    run = wandb.init(
        ...
        config={
            ...
            "TRAIN_MODEL_HCF": TRAIN_MODEL_HCF,
...
if TRAIN_MEMORY_INIT:
    ...
    memory = ReplayMemory(TRAIN_MEMORY_SIZE_LONG)
...
if os.path.isfile("policy_net_states.pt") and os.path.isfile("optimizer_states.pt"):
    ...
    for i in tqdm(range(TRAIN_MODEL_INIT_STEPS),desc="pretrain"):
        ...
        if WANDB_LOGGING and optimize_model_steps % TRAIN_LOG_INTERVAL == 0:
...
for i_episode in tqdm(range(num_episodes), desc="episodes"):
    ...
    if WANDB_LOGGING and i_episode % TRAIN_LOG_INTERVAL == 0:
