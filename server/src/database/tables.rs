...
pub static GAME_MATCH_DB: Lazy<typed_sled::Tree<uuid::Uuid, GameMatch>> =
    Lazy::new(|| {
        let n = format!("game_match_v9_{}_{}", GAME_STATE_VERSION, GAME_MATCH_VERSION);
        typed_sled::Tree::<_, _>::open(&TABLES_DB, n)
    });

pub static GAME_MATCH_FOR_GAME_ID_DB: Lazy<typed_sled::Tree<GameId, uuid::Uuid>> =
    Lazy::new(|| {
        let n = format!("game_match_for_game_id_v7_{}_{}", GAME_STATE_VERSION, GAME_MATCH_VERSION);
        typed_sled::Tree::<_, _>::open(&TABLES_DB, n)
    });

pub static GAME_MATCH_IS_IN_PROGRESS_DB: Lazy<typed_sled::Tree<uuid::Uuid, bool>> =
    Lazy::new(|| {
        let n = format!("game_match_is_in_progress_v7_{}__{}", GAME_STATE_VERSION, GAME_MATCH_VERSION);
        typed_sled::Tree::<_, _>::open(&TABLES_DB, n)
    });
...
> = Lazy::new(|| {
    let n = format!("GAME_MATCHES_FOR_USER_DB_v7_{}_{}", GAME_STATE_VERSION, GAME_MATCH_VERSION);
    typed_sled::Tree::<_, _>::open(&TABLES_DB, n)
});
