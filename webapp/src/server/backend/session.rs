use super::super::api::user::GuestInfo;
...
pub fn make_session_layer() -> SessionManagerLayer<SledStore> {
    use crate::server::database::config::SERVER_DATA_PATH;
...
pub async fn extract_guest_data() -> Result<GuestInfo, ServerFnError> {
    use leptos_axum::extract;
    let s: Guest = extract().await?;
    // s.mark_pageview().await;
    Ok(s.guest_data)
}
