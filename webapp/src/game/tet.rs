...
pub mod tests {
    ...
    pub fn random_have_pinned_results() {
        ...
        let expected_seed = [184, 68, 55, 204, 137, 83, 38, 119, 235, 217, 151, 252, 189, 211, 31, 240, 177, 134, 34, 165, 4, 237, 12, 233, 188, 242, 29, 154, 187, 93, 148, 100];
        assert_eq!(expected_seed, state.seed);

        state.apply_action_if_works(TetAction::HardDrop, 1).unwrap();
        
        let expected_seed = [63, 8, 223, 96, 71, 65, 34, 121, 209, 0, 184, 121, 139, 200, 63, 17, 128, 97, 128, 199, 15, 96, 110, 237, 142, 156, 165, 26, 138, 216, 176, 245];
        assert_eq!(expected_seed, state.seed);
    }
