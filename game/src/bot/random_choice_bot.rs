use crate::{tet::{GameState, Tet, TetAction}, timestamp::get_timestamp_now_ms};
...
pub fn get_all_move_chains() -> Vec<Vec<TetAction>> {
    let rotate_actions = vec![
        ...
        vec![TetAction::Hold,TetAction::RotateRight, TetAction::RotateRight],
    ];
        ...
        for is_left in 0..=1 {
            for move_length in 0..=5 {
                ...
                for _ in 0..move_length {
                    let new_move_action = if is_left == 1 {TetAction::MoveLeft} else {TetAction::MoveRight};
...
fn get_score_for_board(old_state: &GameState, new_state: &GameState) -> anyhow::Result<f64> {
    ...
    Ok(-0.51 * (new_state.main_board.get_height() as f64)
    + 0.76 * (line_diff as f64))
}
