    ...
    for x in (0..b.get_num_cols()).rev() {
        ...
        for y in 0..height {
            match b.v[y as usize][x as usize] {
                crate::tet::CellValue::Empty | crate::tet::CellValue::Ghost => {holes += 1;},
                _ => {},
...
fn get_wordpress_score_for_board(old_state: &GameState, new_state: &GameState) -> anyhow::Result<f64> {
    ...
    + 0.76 * (line_diff as f64) - 0.35 * (holes as f64) - 0.18 * (bumpi as f64))
}
