...
pub fn SoundSettingsTab(user_profile: user::UserProfile, guest_id: GuestInfo) -> impl IntoView {
    ...
    view! {
        <table>
            ...
            // DISBLAE ALL SOUNDS
            <ToggleSettingRow
                signal=user_setting_signal.all_sound_enabled
            ...
            <Show when=move || user_setting_signal.all_sound_enabled.get()>
                <SliderSettingRow
                    signal=user_setting_signal.sound_all_sounds_volume
                    title="Volume".to_string()
                />
                // DISABLE_MENU_MUSIC
                <ToggleSettingRow
                    signal=user_setting_signal.sound_menu_music_enabled
                    title="Menu Music".to_string()
                    on_label="ON".to_string()
                    off_label="OFF".to_string()
                />
                // MMENU MMUSIC VOLUME
                <Show when=move || user_setting_signal.sound_menu_music_enabled.get()>
                    <SliderSettingRow
                        signal=user_setting_signal.sound_menu_music_volume
                        title="Music Volume".to_string()
                    />
                </Show>
            </Show>
        </table>
        ...
        <tr>
            <td style="width:20vmin;">
                <Slider
                    ...
                    max=100.0
                    step=1.0
                    value=signal.read_only()
                    set_value=signal.write_only()
...
pub fn ToggleSettingRow(signal: RwSignal<bool>, title: String, on_label: String, off_label: String)-> impl IntoView{
    ...
    view! {
        <tr>
            <td style="width:20vmin;">
                <Toggle state=signal.read_only() set_state=signal.write_only()/>
            </td>
