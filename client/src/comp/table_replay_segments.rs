use game::{
    ...
    }, random::GameSeed, tet::GameReplaySegment, timestamp::get_human_readable_nano
};
...
#[component]
pub fn TableReplaySegments(all_segments: Signal<Vec<GameReplaySegment>>, slider: RwSignal<f64>) -> impl IntoView {
...
;
    view! {
        <table id="table-replay-segments">
            {make_table}
        </table>
    }
...
impl GameSegmentTableRow {
    pub fn new(row_idx: usize, db_row: GameReplaySegment, current_slider: usize) -> Self {
        let selected = if  row_idx == current_slider {"X".to_string()} else {"".to_string()};
        match db_row {
            GameReplaySegment::Init(_init) => {
                Self {
                    _type:"init".to_owned(),
                    idx: "".to_owned(),
                    action: "".to_owned(),
            ...
            GameReplaySegment::Update(_update) => {
                Self {
